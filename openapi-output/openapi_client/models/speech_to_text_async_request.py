...
class SpeechToTextAsyncRequest(BaseModel):
    ...
    provider_params: Optional[StrictStr] = Field(default=None, description=" Parameters specific to the provider that you want to send along the request.  it should take a *provider* name as key and an object of parameters as value.  Example:      {       \"deepgram\": {         \"filler_words\": true,         \"smart_format\": true,         \"callback\": \"https://webhook.site/0000\"       },       \"assembly\": {         \"webhook_url\": \"https://webhook.site/0000\"       }     }  Please refer to the documentation of each provider to see which parameters to send. ")
    file: Optional[Union[StrictBytes, StrictStr]] = Field(default=None, description="File to analyse in binary format to be used with *content-type*: **multipart/form-data** <br> **Does not work with application/json !**")
    ...
    language: Optional[StrictStr] = Field(default=None, description="Language code expected (ex: en, fr)")
    speakers: Optional[StrictInt] = Field(default=2, description="Number of speakers in the file audio")
    profanity_filter: Optional[StrictBool] = Field(default=False, description="Boolean value to specify weather or not the service will filter profanity and replace inappropriate words with a series of asterisks")
    custom_vocabulary: Optional[StrictStr] = Field(default='', description="List of words or composed words to be detected by the speech to text engine. (Ex: Word, Mike, Draw, Los Angeles,...)")
    convert_to_wav: Optional[StrictBool] = Field(default=False, description="Boolean value to specify weather to convert the audio/video file to wav format to be accepted by a majority of the providers")
    __properties: ClassVar[List[str]] = ["providers", "fallback_providers", "show_original_response", "webhook_receiver", "users_webhook_parameters", "settings", "provider_params", "file", "file_url", "language", "speakers", "profanity_filter", "custom_vocabulary", "convert_to_wav"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if self.profanity_filter is None and "profanity_filter" in self.model_fields_set:
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "speakers": obj.get("speakers") if obj.get("speakers") is not None else 2,
            "profanity_filter": obj.get("profanity_filter") if obj.get("profanity_filter") is not None else False,
            "custom_vocabulary": obj.get("custom_vocabulary") if obj.get("custom_vocabulary") is not None else '',
            "convert_to_wav": obj.get("convert_to_wav") if obj.get("convert_to_wav") is not None else False
        })
        return _obj
