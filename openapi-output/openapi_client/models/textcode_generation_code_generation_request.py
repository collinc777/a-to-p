...
class TextcodeGenerationCodeGenerationRequest(BaseModel):
    """
    ...
    prompt: Optional[StrictStr] = Field(default='', description="Entrer the source code that will be used as a context.")
    instruction: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Entrer the instruction you want to be followed.")
    ...
    max_tokens: Optional[Annotated[int, Field(le=4096, strict=True, ge=1)]] = Field(default=1000, description="The maximum number of tokens to generate in the completion. The token count of your prompt plus max_tokens cannot exceed the model's context length.")
    __properties: ClassVar[List[str]] = ["providers", "fallback_providers", "response_as_dict", "attributes_as_list", "show_original_response", "prompt", "instruction", "temperature", "max_tokens"]
