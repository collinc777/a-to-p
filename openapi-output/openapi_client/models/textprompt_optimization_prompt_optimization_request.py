...
class TextpromptOptimizationPromptOptimizationRequest(BaseModel):
    ...
    text: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Description of the desired prompt.")
    target_provider: TargetProviderEnum = Field(description="Select the provider for the prompt optimization  * `google` - google * `openai` - openai * `cohere` - cohere")
    __properties: ClassVar[List[str]] = ["providers", "fallback_providers", "response_as_dict", "attributes_as_list", "show_original_response", "text", "target_provider"]
