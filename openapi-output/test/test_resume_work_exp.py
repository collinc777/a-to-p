# coding: utf-8

"""
    Eden AI

    Your project description

    The version of the OpenAPI document: 2.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.resume_work_exp import ResumeWorkExp

class TestResumeWorkExp(unittest.TestCase):
    """ResumeWorkExp unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ResumeWorkExp:
        """Test ResumeWorkExp
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ResumeWorkExp`
        """
        model = ResumeWorkExp()
        if include_optional:
            return ResumeWorkExp(
                total_years_experience = '',
                entries = [
                    openapi_client.models.resume_work_exp_entry.ResumeWorkExpEntry(
                        title = '', 
                        start_date = '', 
                        end_date = '', 
                        company = '', 
                        location = openapi_client.models.resume_location.ResumeLocation(
                            formatted_location = '', 
                            postal_code = '', 
                            region = '', 
                            country = '', 
                            country_code = '', 
                            raw_input_location = '', 
                            street = '', 
                            street_number = '', 
                            appartment_number = '', 
                            city = '', ), 
                        description = '', 
                        industry = '', )
                    ]
            )
        else:
            return ResumeWorkExp(
                total_years_experience = '',
        )
        """

    def testResumeWorkExp(self):
        """Test ResumeWorkExp"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
