...
class FinancialParserObjectDataClass(BaseModel):
    ...
    item_lines: Optional[List[FinancialLineItem]] = Field(
        default=None, description="List of line items associated with the document."
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.financial_document_information:
            _dict["financial_document_information"] = (
                self.financial_document_information.to_dict()
            )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "customer_information": FinancialCustomerInformation.from_dict(
                    obj["customer_information"]
                )
                if obj.get("customer_information") is not None
                else None,
                "merchant_information": FinancialMerchantInformation.from_dict(
                    obj["merchant_information"]
                )
                if obj.get("merchant_information") is not None
                else None,
                "payment_information": FinancialPaymentInformation.from_dict(
                    obj["payment_information"]
                )
                if obj.get("payment_information") is not None
                else None,
                "financial_document_information": FinancialDocumentInformation.from_dict(
                    obj["financial_document_information"]
                )
                if obj.get("financial_document_information") is not None
                else None,
                "local": FinancialLocalInformation.from_dict(obj["local"])
                if obj.get("local") is not None
                else None,
                "bank": FinancialBankInformation.from_dict(obj["bank"])
                if obj.get("bank") is not None
                else None,
                "item_lines": [
                    FinancialLineItem.from_dict(_item) for _item in obj["item_lines"]
                ]
                if obj.get("item_lines") is not None
                else None,
                "document_metadata": FinancialDocumentMetadata.from_dict(
                    obj["document_metadata"]
                )
                if obj.get("document_metadata") is not None
                else None,
            }
        )
        return _obj
