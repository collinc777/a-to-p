...
class OcrfinancialParserFinancialParserRequest(BaseModel):
    ...
    document_type: Optional[DocumentTypeEnum] = Field(
        default=None,
        description="Specify the type of your document. Can be Set to 'auto-detect' for automatic detection if the provider supports it. Otherwise, the default is 'invoice'.  * `auto-detect` - auto-detect * `invoice` - invoice * `receipt` - receipt",
    )
